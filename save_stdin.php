...
try {
	...
	$output_file = empty($argv[1]) ? dirname(__FILE__) . '/aaa_stdin_data_capture_' . microtime(true) . '.' . mt_rand(9999, 99999) . '.log' : $argv[1];

	$out = fopen($output_file, 'wb');

	if (empty($out)) {
		throw new Exception("Cannot open output file [$output_file] for writing.");
	}
